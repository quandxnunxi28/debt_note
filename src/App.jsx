...
import { Search, PlusCircle, MapPin, User, FileText, CheckCircle, Wallet, Trash2, X, Filter } from 'lucide-react';
...
export default function DebtTracker() {
  ...
  // 4. LỌC VÀ TÌM KIẾM (ĐÃ NÂNG CẤP TÌM KHÔNG DẤU)
  const filteredDebts = useMemo(() => {
    return debts.filter(debt => {
      // Chuẩn hóa từ khóa tìm kiếm và tên trong danh sách về dạng KHÔNG DẤU & CHỮ THƯỜNG
      ...
      // Lọc theo danh sách thả xuống
      const debtLoc = formatLocationName(debt.location);
      const matchFilter = filterLocation === 'Tất cả' || debtLoc === filterLocation;
      
      return matchSearch && matchFilter; 
    });
  }, [debts, searchTerm, filterLocation]);
    ...
    <div style={styles.container}>
      ...
      <div style={styles.controlPanel}>
        <div style={styles.searchRow}>
          ...
          />
          {searchTerm && (
            <button onClick={() => setSearchTerm('')} style={{background:'none', border:'none', padding:0}}>
        ...
        <div style={styles.filterRow}>
          <Filter style={{ color: '#10b981', marginRight: '10px' }} size={24} />
          <select 
      ...
      <div style={styles.listContainer}>
        {currentDebts.length === 0 ? (
          ...
          currentDebts.map(debt => (
            <div key={debt.id} style={{...styles.card, opacity: debt.isPaid ? 0.6 : 1}}>
...
const styles = {
  ...
  controlPanel: { backgroundColor: '#fff', margin: '20px', padding: '15px', borderRadius: '20px', border: '1px solid #e5e7eb', boxShadow: '0 4px 15px rgba(0,0,0,0.05)' },
  ...
  selectInput: { border: 'none', outline: 'none', width: '100%', fontSize: '1.1rem', color: '#10b981', fontWeight: 'bold', backgroundColor: 'transparent', cursor: 'pointer' },

  listContainer: { padding: '10px 20px', display: 'flex', flexDirection: 'column', gap: '20px' },
  card: { backgroundColor: '#fff', padding: '25px', borderRadius: '20px', border: '1px solid #e5e7eb', boxShadow: '0 4px 6px rgba(0,0,0,0.05)', transition: '0.3s' },
